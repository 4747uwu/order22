...
function deriveFileNameFromUrl(zipUrl, fallback = 'study.zip') {
  try {
    const u = new URL(zipUrl);
    const fromDisp = u.searchParams.get('response-content-disposition');
    if (fromDisp) {
      const utf8 = fromDisp.match(/filename\*=UTF-8''([^;]+)/i);
      if (utf8?.[1]) return decodeURIComponent(utf8[1]);
      const basic = fromDisp.match(/filename="?([^";]+)"?/i);
      if (basic?.[1]) return basic[1];
    }
    const base = decodeURIComponent(path.basename(u.pathname || '') || '');
    if (base && base.toLowerCase().endsWith('.zip')) return base;
  } catch (_) {}
  return fallback;
}
...
// ── Refresh expired presigned download URL using stored zipKey ────────────────
