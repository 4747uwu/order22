...
const Dashboard = () => {
  ...
  // ✅ FIX: Ensure fetchStudies always uses current filters
  const fetchStudies = useCallback(async (filters = {}) => {
    setLoading(true);
    setError(null);
    try {
      const endpoint = getApiEndpoint();
      // ✅ CRITICAL: Use current searchFilters if no filters provided
  ...
  }, [getApiEndpoint, currentView, searchFilters]); // ✅ ADD searchFilters to dependencies
  ...
  // ✅ FIX: Handle view change with preserved filters
  const handleViewChange = useCallback((view) => {
    ...
    const preservedFilters = {
      ...searchFilters, // ✅ Keep ALL existing filters
      category: view === 'all' ? undefined : view
    };
    
    console.log('🔄 [Admin] PRESERVING FILTERS FOR TAB CHANGE:', {
      currentFilters: searchFilters,
      preservedFilters,
      newView: view
    });
    
    // ✅ IMMEDIATE: Update both analytics and studies
    fetchAnalytics(preservedFilters);
    fetchStudies(preservedFilters);
    
    // Update state
    setSearchFilters(preservedFilters);
  }, [currentView, searchFilters, fetchAnalytics, fetchStudies]);
