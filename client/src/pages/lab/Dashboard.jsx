...
const LabDashboard = () => {
  ...
  const getDefaultColumnConfig = () => ({
    ...
    studyLock:           { visible: false, order: 18, label: 'Lock/Unlock' },
    status:              { visible: true,  order: 19, label: 'Status' },
    printCount:          { visible: true,  order: 20, label: 'Print Report' },
    rejectionReason:     { visible: false, order: 21, label: 'Reverted Reason' },
    assignedVerifier:    { visible: false, order: 22, label: 'Finalised By' },
    verifiedDateTime:    { visible: false, order: 23, label: 'Finalised Date/Time' },
    ...
    try {
      ...
      if (saved) {
        ...
        if ('checkbox' in parsedConfig) {
          console.warn('🔄 Migrating stale lab column config — resetting');
          localStorage.removeItem('labWorklistColumnConfig');
