...
const UnifiedStudyRow = ({
    ...
    const [wabaModal, setWabaModal] = useState(false);
    ...
    return (
        ...
        <td className="px-3 py-3.5 text-center border-r border-b border-slate-200 align-center" style={{ width: `${getColumnWidth('reporting')}px` }}>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-1 max-w-[100px] mx-auto">
                        ...
                        {!userAccountRoles.includes('super_admin') && !userAccountRoles.includes('verifier') && (
                            <>
                                ...
                                <button
                                    onClick={() => setWabaModal(true)}
                                    className="p-1 hover:bg-green-50 rounded transition-all hover:scale-110"
                                    title="Send WhatsApp Notification"
                                >
                                    <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-[#25D366]" fill="currentColor">
            ...
            {wabaModal && <WhatsAppModal study={study} onClose={() => setWabaModal(false)} />}
        </tr>
    );
};
            ...
            <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="text-[10px] text-slate-500">
                        For any query call: <span className="font-semibold text-teal-600">{supportNumber || '98XXXX'}</span>
                    </div>
                </div>
            </div>
        </div>
...
const UnifiedWorklistTable = ({
    ...
    const handleBulkVerify = useCallback(async () => {
        ...
        try {
            const res = await api.post('/verifier/studies/bulk-verify', {
                studyIds: selectedStudies,
                approved: true,
                verificationNotes: 'Bulk verified from worklist'
            });
            if (res.data?.success) {
                const { successCount, failCount, total } = res.data.data;
                toast.success(`Verified ${successCount}/${total} studies${failCount > 0 ? ` (${failCount} failed)` : ''}`);
                selectedStudies.forEach(id => onSelectStudy?.(id));
                onRefreshStudies?.();
            } else {
                toast.error(res.data?.message || 'Bulk verify failed');
            }
        } catch (error) {
            toast.error(`Bulk verify error: ${error.message}`);
        } finally {
                ...
                <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 bg-blue-600 text-white text-[10px] sm:text-xs font-semibold border-b border-blue-700 flex-shrink-0 z-20">
                    ...
                    {(userAccountRoles.includes('verifier') || userAccountRoles.includes('admin') || userAccountRoles.includes('assignor')) && (
                        <button
                            ...
                            className="flex items-center gap-1 px-2 py-1 bg-green-500 text-white rounded hover:bg-green-400 transition-colors font-bold disabled:opacity-50"
                        >
                            {bulkVerifying ? <div className="animate-spin rounded-full h-3 w-3 border border-white border-t-transparent" /> : <CheckCircle className="w-3 h-3" />}
