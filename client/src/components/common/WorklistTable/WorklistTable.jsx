...
import { Copy, UserPlus, Lock, Unlock, Edit, Clock, Download, Paperclip, MessageSquare, FileText, Monitor, Eye, ChevronLeft, ChevronRight, CheckCircle, XCircle, Palette, Share2, RotateCcw, Printer, Users } from 'lucide-react';
...
const getPriorityWeight = (study) => {
  ...
  return PRIORITY_SORT_ORDER[p] ?? 3;
};
    ...
    {isColumnVisible('caseStatus') && (
        <td className="px-1.5 py-2 sm:px-2 text-center border-r border-slate-200 align-middle" style={{ width: `${getColumnWidth('status')}px` }}>
            <div className="flex flex-col items-center gap-1">
                <span className={`inline-block px-1.5 py-0.5 rounded text-[8px] sm:text-[9px] font-medium whitespace-normal break-words leading-tight ${getStatusColor(study.workflowStatus)}`}>{study.caseStatusCategory || formatWorkflowStatus(study.workflowStatus)}</span>
                <span className="text-[9px] sm:text-[10px] text-slate-600 whitespace-normal break-words leading-tight">{study.workflowStatus ? formatWorkflowStatus(study.workflowStatus) : '-'}</span>
            </div>
        </td>
    )}
    ...
    {isColumnVisible('actions') && (
        <td className="px-1.5 py-2 sm:px-2 text-center border-slate-200 align-middle" style={{ width: `${getColumnWidth('actions')}px` }}>
            ...
            <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-700 group-hover:text-gray-900" />
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/study/${study._id}`); toast.success('Link copied!'); }} className="p-1 hover:bg-sky-50 rounded" title="Share Study"><Share2 className="w-3.5 h-3.5 text-sky-600" /></button>

                    {userAccountRoles.includes('admin') && (
                        <>
                            <button className="px-1.5 py-0.5 text-[8px] sm:text-[9px] font-semibold bg-green-600 text-white rounded hover:bg-green-700" title="Verify" onClick={() => window.open(`/online-reporting/${study._id}?openOHIF=true&verifierMode=true&action=verify`, '_blank')}>Verify</button>
                            {study.workflowStatus === 'report_verified' && <CheckCircle className="w-3 h-3 text-green-600" />}
      ...
      case 'study_viewer_opened':
        setActiveViewers(prev => {
          ...
          if (!viewers.find(v => v.userId === message.data.userId)) {
            return { ...prev, [studyId]: [...viewers, { userId: message.data.userId, userName: message.data.userName, mode: message.data.mode, openedAt: new Date().toISOString() }] };
          }
          return prev;
        });
        break;
